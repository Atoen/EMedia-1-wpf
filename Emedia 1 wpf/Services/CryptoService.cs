...
public class CryptoService
{
    ...
    public const int EncryptChunkSize = KeySize / 8 - 11;
    public const int DecryptChunkSize = KeySize / 8;
    
    private const double UpdateThreshold = 0.005;

    private CustomRSA? _customRSA;
    
    public CryptoService()
    {
        var privateKey64 = File.ReadAllText("keypair.pem");
    ...
    {
        if (cryptographyMode == CryptographyMode.Library)
        {
            return await Task.Run(() => EncryptLibrary(data, progress));
        }

        _customRSA ??= await CreateRSA(data);

        return await Task.Run(() => cryptographyMode == CryptographyMode.ECB
            ? _customRSA.EncryptECB(data, progress)
            : _customRSA.EncryptCBC(data, progress));
    }
        ...
        IProgress<double>? progress = null)
    {
        if (cryptographyMode == CryptographyMode.Library)
        {
            return await Task.Run(() => DecryptLibrary(encryptedData, progress));
        }

        // var m = await Task.Run(() =>
        // {
        //     const int step = CustomRSA.KeySize / 8 - 1;
        //     return encryptedData.Chunk(step)
        //         .Select(x => new BigInteger(x))
        //         .Max();
        // });

        if (_customRSA is null)
        {
            throw new InvalidOperationException("Custom RSA is not initialized");
        }
        
        return await Task.Run(() => cryptographyMode == CryptographyMode.ECB
            ? _customRSA.DecryptECB(encryptedData, progress)
            : _customRSA.DecryptCBC(encryptedData, progress));
    }

    private async Task<CustomRSA> CreateRSA(IEnumerable<byte> data)
    {
        var m = await Task.Run(() =>
        {
            const int step = CustomRSA.KeySize / 8 - 1;
            return data.Chunk(step)
                .Select(x => new BigInteger(x, isBigEndian: true, isUnsigned: true))
                .Max();
        });

        return new CustomRSA(m);
    }
    ...
    private byte[] DecryptLibrary(IEnumerable<byte> encryptedData, IProgress<double>? progress = null)
    {
        using var csp = new RSACryptoServiceProvider();
    ...
    //             var m = new BigInteger(x, isUnsigned: true, isBigEndian: true); // Convert chunk to BigInteger
    //             var c = BigInteger.ModPow(m, e, n); // c = m^e % n
    //
    //             var encryptedChunk = c.ToByteArray(isUnsigned: false, isUnsigned: true, isBigEndian: true); // Convert back to byte array and reverse
    //
    ...
    //     {
    //         var m = new BigInteger(x, isUnsigned: true, isBigEndian: true);
